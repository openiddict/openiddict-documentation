...
```xml
<ItemGroup>
  <PackageReference Include="OpenIddict.AspNetCore" Version="5.8.0" /> <!-- [!code remove] -->
  <PackageReference Include="OpenIddict.AspNetCore" Version="6.4.0" /> <!-- [!code add] -->

  <PackageReference Include="OpenIddict.EntityFrameworkCore" Version="5.8.0" /> <!-- [!code remove] -->
  <PackageReference Include="OpenIddict.EntityFrameworkCore" Version="6.4.0" /> <!-- [!code add] -->
</ItemGroup>
```
...
As part of this change, the `OpenIddictConstants.Prompts` class has been renamed to `OpenIddictConstants.PromptValues` and the
...
## Migrate to `MongoDB.Driver` version 3.0 (if applicable)

To fix a breaking change introduced by MongoDB in their 2.x branch, the `OpenIddict.MongoDb` and `OpenIddict.MongoDb.Models`
packages now reference `MongoDB.Driver` and `MongoDB.Bson` version 3.0.0 and are now strong-named: OpenIddict users relying
on the MongoDB integration will also need to update their MongoDB driver dependencies when upgrading OpenIddict to 6.0.
...
## Update your checks to ensure authenticated identities are correctly identified (OWIN only)
